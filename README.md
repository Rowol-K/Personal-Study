...
단계별 성장을 기록하고 기능 단위 개발을 실천하기 위해 체계적인 브랜치 전략을 사용했습니다.

* **`master`**: 리팩토링 전, **C언어 레거시 원본 코드** 보존.
* **`refactor/oop`**: **C++ 객체지향 리팩토링** 작업 완료 버전 (Engine/Manager 분리).
* **`feat/socket-basic`**: **소켓 통신 기초 및 멀티스레드** 기능 구현 버전.
* **`feat/NB_Engine`**: **게임 로직(GameEngine) 연동**, 패킷 처리 및 브로드캐스팅 시스템 구현 버전.
* **`main`**: 모든 기능이 통합되고 테스트가 완료된 **최종 배포 버전**.
